...
void Renderer::draw_gradient_rect(const RenderListPtr &render_list, const Vec4 &rect, Color color1, Color color2)
{
	Vertex v[] = {
	{ rect.x, rect.y, 0.0f, color1 },
	{ rect.x + rect.z, rect.y, 0.0f, color1 },
	{ rect.x, rect.y + rect.w, 0.0f, color2 },
	{ rect.x + rect.z, rect.y + rect.w, 0.0f, color2 },
	{ rect.x, rect.y + rect.w, 0.0f, color2 },
	{ rect.x + rect.z, rect.y, 0.0f, color1 }
	};
...
void Renderer::draw_gradient_rect(const Vec4 &rect, Color color1, Color color2)
{
	draw_gradient_rect(render_list, rect, color1, color2);
}
...
void Renderer::draw_text(const RenderListPtr &render_list, FontHandle font, Vec2 position, const std::string &text, Color color, std::uint8_t flags)
{
	if (font.id < 0 || font.id >= std::size(fonts))
		throw std::exception("bad font handle");//fmt::format("Renderer::draw_text: Bad font handle (identifier: {})!", font.id).c_str());
...
void throw_if_failed(HRESULT hr)
{
	if (FAILED(hr))
	{
		throw std::exception("crucial direct3d 9 operation failed");// fmt::format("Crucial Direct3D 9 operation failed! Code: %X", hr).c_str());
	}
}